...
const addCars = (car, brand) => {
  ...
  const BRANDS = utils.brands;

  if (CARS.find((auto) => auto.id === car.id)) {
    throw Error("Este coche ya existe en la base de datos");
  }

  if (BRANDS.find((marca) => marca.name === brand.name)) {
    throw Error("Esta marca ya existe en la base de datos");
  }
