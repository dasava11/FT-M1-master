...
const getCarsByBrand = (brand, sort) => {
  const marca = utils.brands.find((marca) => marca.name === brand);

  if (!marca) {
    throw new Error("Marca no encontrada");
  }

  if (marca.cars.length === 0) {
    return "No se encontraron coches";
  }

  const finded = utils.cars.filter((car) => marca.cars.includes(car.id));
  if (sort === "highPrice") {
    finded.sort((a, b) => b.price - a.price);
  }

  if (sort === "lowPrice") {
    finded.sort((a, b) => a.price - b.price);
  }
